...
/// <summary>
/// Centralizes Radarr HTTP access with shared headers, JSON options, and retry handling.
/// </summary>
public class RadarrApiClient
{
    ...
    public RadarrApiClient(HttpClient httpClient, IOptions<AppSettings> appSettings, ILogger<RadarrApiClient> logger)
    {
        _httpClient = httpClient;
        _logger = logger;
        _settings = appSettings.Value.Radarr;

        if (string.IsNullOrWhiteSpace(_settings.Url))
            throw new InvalidOperationException("RADARR_URL not configured");
        if (string.IsNullOrWhiteSpace(_settings.ApiKey))
            throw new InvalidOperationException("RADARR_API_KEY not configured");
        ...
        CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Sending Radarr request: {Method} {Endpoint}", method, relativeEndpoint);
